...
- `mepdmsnapshot` writes that same machine-readable structured DM snapshot to disk with a required label such as `start`, `mid`, or `end`. Use `--out` when you need an exact path under the current working directory; otherwise the adapter writes a sanitized default file such as `soak-<context>-<label>.json`.
...
- `--out must stay within the current working directory`: write the snapshot to a relative path such as `evidence/start.json` or an absolute path rooted under the directory where the adapter is running.
