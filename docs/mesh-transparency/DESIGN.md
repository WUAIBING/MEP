...
2. Expose, do not force: visibility is available, not mandatory.
3. Seeing is not controlling: transparency and control are separate layers.
...
- DMs between nodes are not shown; privacy is preserved.
...
|-------|---------------|---------------|
| Node joined mesh | yes | yes |
| Node left mesh | yes | yes |
| Task assigned to node | yes | yes |
| Task completed | yes | yes |
| Node reputation changed | yes | yes |
| Node blocked/slashed | yes | yes |
| DM sent (metadata only: from/to/timestamp) | yes | yes |
| DM content | no | no |
| Broadcast received | yes | yes |
| Bid submitted | yes | yes |
| Error / failure | yes | yes |
...
1. Trust through transparency: "you can check whenever you want" signals that MEP is not hiding anything.
